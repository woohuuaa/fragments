...
async function deleteFragment(ownerId, id) {
  ...
  // Create a DELETE Object command to send to S3 and a DELETE command to send to DynamoDB
  const s3Command = new DeleteObjectCommand(s3Params);
  const ddbCommand = new DeleteCommand(ddbParams);

  try {
    // Delete metadata and raw fragment data concurrently
    await Promise.all([ddbDocClient.send(ddbCommand), s3Client.send(s3Command)]);
  } catch (err) {
    const { TableName } = ddbParams;
    const { Bucket, Key } = s3Params;
    logger.error(
      { err, ownerId, id, TableName, Bucket, Key },
      'Error deleting fragment metadata or data'
    );
